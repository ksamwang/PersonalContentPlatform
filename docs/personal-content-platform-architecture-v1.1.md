...
Docker Compose
  ...
  api               Go
  worker-default    Go
  worker-media      Go (resource limits)
  postgres          PostgreSQL + pgvector
  otel-collector    traces / metrics / logs

External: S3/R2 object storage + CDN + AI/providers
```
...
- dev/staging/prod 使用同一镜像，不同配置；数据库和对象存储隔离。
...
| Proxy / CDN         | Caddy + Cloudflare      | TLS、缓存、WAF                                  |
| Observability       | OpenTelemetry           | 导出到可替换后端                                |
| CI/CD               | GitHub Actions          | 测试、构建、迁移检查、镜像与部署                |

...
3. 创建仅 root 可读的 `/etc/pcplatform/pcplatform.env`。生产数据库必须使用 TLS 或受限内网连接；配置 `PUBLIC_WEB_ORIGIN`，并让 API、Worker 和 Public Web 使用相同的 `PUBLIC_REVALIDATE_TOKEN`。
4. 执行 `pcp-migrate up`，确认成功后替换 API/Worker。
5. 安装 `deploy/systemd` 下的 unit，执行 daemon-reload、enable 和 restart。
6. 检查 `/healthz`、`/readyz`、结构化访问日志、Worker 的 Outbox/Jobs 日志、发布队列和公开页面。

...
export function DiscoverPanel({workspace,role,onOpen}:{workspace:string;role:string;onOpen:(content:Content)=>void}){
  ...
  return <section className="discover-page"><header className="page-heading"><div><span className="eyebrow">DISCOVER</span><h1>发现与问答</h1><p>结合 PostgreSQL 全文检索与向量语义，找回分散在内容中的信息。</p></div>{canEdit&&<button className="secondary" disabled={indexing} onClick={()=>void rebuild()}><RefreshCw aria-hidden size={16}/>{indexing?"重建中…":"重建索引"}</button>}</header>

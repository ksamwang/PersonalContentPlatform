...
export function ProviderSettingsForm({workspace,storage,setStorage,ai,setAI,canEdit}:{workspace:string;storage:StorageSettings;setStorage:(v:StorageSettings)=>void;ai:AISettings;setAI:(v:AISettings)=>void;canEdit:boolean}){
 const [accessKey,setAccessKey]=useState(""),[secretKey,setSecretKey]=useState(""),[apiKey,setAPIKey]=useState(""),[probe,setProbe]=useState("");
 ...
 return <>
  <SettingsSection title="对象存储" description="新上传使用当前配置；旧文件继续绑定原有 Provider。" canEdit={canEdit} onSave={async()=>{const saved=await api.saveStorage(workspace,{...storage,access_key:accessKey,secret_key:secretKey});setStorage(saved);setAccessKey("");setSecretKey("")}}>
   ...
   {storage.provider==="filesystem"?<Field label="服务器存储目录"><input disabled={!canEdit} value={storage.base_path} onChange={e=>setS("base_path",e.target.value)}/></Field>:<>
   ...
   {canEdit&&<button className="secondary settings-test" onClick={async()=>{setProbe("测试中…");try{await api.testStorage(workspace);setProbe("连接成功")}catch(e){setProbe(e instanceof Error?e.message:"连接失败")}}}>测试连接</button>}<span aria-live="polite">{probe}</span>
  </SettingsSection>
  <SettingsSection title="AI Provider" description="支持 OpenAI 兼容接口，可为不同用途指定模型。" canEdit={canEdit} onSave={async()=>{const saved=await api.saveAI(workspace,{...ai,api_key:apiKey});setAI(saved);setAPIKey("")}}>
   ...
   {canEdit&&<button className="secondary settings-test" onClick={async()=>{setProbe("测试中…");try{await api.testAI(workspace);setProbe("连接成功")}catch(e){setProbe(e instanceof Error?e.message:"连接失败")}}}>测试连接</button>}
  </SettingsSection>
 </>
}

...
export function SettingsPanel({workspace,role}:{workspace:string;role:string}){
 ...
 return <WorkspacePage className="settings-page" eyebrow="WORKSPACE CONTROL" title="工作区设置" description={canEdit?"配置内容站点、Provider 与发布行为。":"你可以查看配置，只有 Owner 可以修改。"}><div className="settings-layout"><nav className="settings-nav" aria-label="设置目录"><a href="#settings-workspace">工作区与站点</a><a href="#settings-storage">对象存储</a><a href="#settings-ai">AI 创作</a><a href="#settings-embedding">语义检索</a><a href="#settings-media">媒体处理</a><a href="#settings-webhook">Webhook</a></nav><div className="settings-content"><GeneralSettingsForm value={general} setValue={setGeneral} canEdit={canEdit} save={async section=>{const saved=await api.saveSettings(workspace,section,general[section]);setGeneral(saved)}}/><ProviderSettingsForm workspace={workspace} storage={storage} setStorage={setStorage} ai={ai} setAI={setAI} canEdit={canEdit}/><EmbeddingSettingsForm workspace={workspace} value={embedding} setValue={setEmbedding} canEdit={canEdit}/><MediaProviderSettings workspace={workspace} values={media} setValues={setMedia} canEdit={canEdit}/><WebhookSettings workspace={workspace} canEdit={canEdit}/></div></div></WorkspacePage>
}

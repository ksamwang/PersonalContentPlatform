...
export function CollectionsPanel({workspace,role,onOpenContent}:{workspace:string;role:string;onOpenContent:(id:string)=>void}){
 ...
 return <section className="collections-page"><header className="page-heading"><div><span className="eyebrow">CURATED SETS</span><h1>合集</h1><p>按主题、路径或展示顺序，把已有内容编排成可维护的集合。</p></div>{canEdit&&<button className="primary compact" onClick={()=>setOpen(true)}><Plus aria-hidden size={18}/>新建合集</button>}</header>

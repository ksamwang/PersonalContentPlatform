...
export type SaveEmbeddingSettings = Pick<EmbeddingSettings,"provider"|"base_url"|"model"|"dimensions"> & {api_key?:string};
...
export const api = {
  ...
  translateLocalization: (ws:string,targetID:string,sourceID:string) =>
    request<{job_id:string;state:string}>(`/v1/workspaces/${ws}/localizations/${targetID}:translate`,{method:"POST",body:JSON.stringify({source_localization_id:sourceID})}),
  ...
  saveEmbedding:(ws:string,value:SaveEmbeddingSettings)=>request<EmbeddingSettings>(`/v1/workspaces/${ws}/settings/embedding`,{method:"PUT",body:JSON.stringify(value)}),
